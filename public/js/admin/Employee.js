...
window.openEditModal = function(employeeId, employeeName, email, contactNo, employeeType, role, departmentId, adminId, employeeStatus, paidStatus, profilePhotoUrl) {
...
document.addEventListener('keydown', function(e) {
    if (e.key === 'Escape') {
        ...
        else if (editModal.classList.contains('active')) window.closeEditModal();
    }
});
...
window.searchEmployees = function() {
    ...
    if (foundEmployee) {
        window.showEmployeeLeaveInfo(foundEmployee.id, foundEmployee.name, foundEmployee.department, foundEmployee.position);
    } else {
...
window.openViewModal = function(employeeId, employeeName, email, contactNo, employeeType, role, department, admin, employeeStatus, paidStatus, createdAt, teams, profilePhotoUrl) {
    ...
    if (employeeStatus) {
        ...
        switch(employeeStatus.toLowerCase()) {
            ...
            case 'on leave': statusClass = 'on-leave'; break;
    ...
    if (paidStatus) {
        ...
        switch(paidStatus.toLowerCase()) {
            case 'paid': paidClass = 'paid'; break;
            case 'pending': paidClass = 'pending'; break;
            case 'overdue': paidClass = 'overdue'; break;
            case 'not applicable': paidClass = 'not-applicable'; break;
            default: paidClass = 'pending';
        }
...
document.addEventListener('keydown', function(e) {
    if (e.key === 'Escape') {
        if (document.getElementById('viewEmployeeModal').classList.contains('active')) window.closeViewModal();
    }
});
