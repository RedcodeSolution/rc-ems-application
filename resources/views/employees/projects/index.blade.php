        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="project-meta" style="margin-top:1rem;">
                    ...
                    <span style="display:block;margin-top:.5rem;"><i class="fas fa-calendar"></i> <strong id="viewProjectDeadline">Due: --</strong></span>
                    <span style="display:block;margin-top:.5rem;"><i class="fas fa-flag"></i> <strong id="viewProjectPriority">Priority</strong></span>
                </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <textarea id="editProjectDescription" class="form-control" rows="3" placeholder="Enter project description..."></textarea>
                </div>
                <div class="form-row">
                    <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <div class="range-wrapper" title="Scroll while hovering to change value">
                        <input type="range" id="editProjectProgress" class="form-control range-input" min="0" max="100" step="1" value="0" />
...
<script>
    ...
    function viewProject(projectName) {
        ...
        document.getElementById('viewProjectTeam').textContent = (card.dataset.team || '').replace(/^\w/, c=>c.toUpperCase());
        const deadlineSpan = Array.from(card.querySelectorAll('.project-meta span')).find(s => s.textContent.toLowerCase().includes('due') || s.textContent.toLowerCase().includes('completed'));
        document.getElementById('viewProjectDeadline').textContent = deadlineSpan ? deadlineSpan.textContent : 'Due: --';
    ...
    function saveProjectEdits() {
        ...
        const newProgress = Math.min(100, Math.max(0, parseInt(document.getElementById('editProjectProgress').value || 0)));
        ...
        if (statusEl) {
            ...
            const iconClass = newStatus === 'active' ? 'fa-play' : newStatus === 'completed' ? 'fa-check' : newStatus === 'pending' ? 'fa-pause' : 'fa-exclamation';
            statusEl.innerHTML = `<i class="fas ${iconClass}"></i> ${newStatus.charAt(0).toUpperCase() + newStatus.slice(1)}`;
        }

        const metaSpans = Array.from(card.querySelectorAll('.project-meta span'));
        if (metaSpans.length >= 2) {
            metaSpans[1].innerHTML = `<i class="fas fa-calendar"></i> ${ newDeadline ? ('Due: ' + new Date(newDeadline).toLocaleDateString()) : 'Due: --' }`;
        }
    ...
    (function() {
        ...
        if (typeof origPopulate === 'function') {
            window.populateEditModalFromCard = function(card) {
                origPopulate(card);
                const v = document.getElementById('editProjectProgress') ? document.getElementById('editProjectProgress').value : 0;
                setLabel(v);
            };
        }
