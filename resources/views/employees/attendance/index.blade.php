            ...
            <div class="status-card active" id="todayStatus">
                ...
                <div class="status-info">
                    ...
                    <div class="work-hours">
                        <span>Work Hours: <strong id="workHours">7h 45m</strong></span>
                    </div>
                </div>
            </div>
    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            ...
            <div class="form-group">
                ...
                <textarea id="emergencyDescription" class="form-control" rows="3" placeholder="Provide additional details about the emergency..."></textarea>
            </div>
        </div>
        <div class="modal-footer">
...
<script>
    ...
    function startBreak() {
        if (!isOnBreak) {
            ...
            showMessage('Break started!', 'info');
        }
    }
    ...
    function endEmergencyBreak() {
        if (window.emergencyBreaks && window.emergencyBreaks.length > 0) {
            const lastEmergency = window.emergencyBreaks[window.emergencyBreaks.length - 1];
    ...
    function submitEmergencyReport() {
        ...
        if (!emergencyType) {
            showMessage('Please select an emergency type', 'error');
            return;
        }
        ...
        if (!window.emergencyReports) {
            window.emergencyReports = [];
        }
        window.emergencyReports.push(emergencyData);

        // Create emergency break with default 30-minute duration
        ...
        if (!window.emergencyBreaks) {
            window.emergencyBreaks = [];
        }
        window.emergencyBreaks.push(emergencyBreak);

        // Auto-end emergency break after 30 minutes
        setTimeout(() => {
            endEmergencyBreak();
        }, 30 * 60 * 1000);

        showMessage(`Emergency break started due to: ${emergencyType}`, 'warning');
        ...
        // Log for admin notification (in real implementation, this would be an API call)
        console.log('Emergency Report:', emergencyData);

        // Store in localStorage for persistence
        localStorage.setItem('emergencyReports', JSON.stringify(window.emergencyReports));
        localStorage.setItem('emergencyBreaks', JSON.stringify(window.emergencyBreaks));
    }
