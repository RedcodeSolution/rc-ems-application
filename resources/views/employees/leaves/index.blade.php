                            ...
                            <tbody>
                                @forelse ($leaves as $leave)
                                    <tr>
                                        ...
                                        <td><span
                                                class="status-pill {{ $leave->status }}">{{ ucfirst($leave->status) }}</span>
                                        </td>
                                        <td class="history-actions">
                                            <button class="action-btn view"
                                                onclick="viewLeaveRequest('{{ $leave->leave_id }}')"
                                                title="View Details"><i class="fas fa-eye"></i></button>
                                            @if ($leave->status === 'pending')
                                                <button class="action-btn edit"
                                                    onclick="editLeaveRequest('{{ $leave->leave_id }}')"
                                                    title="Edit Request"><i class="fas fa-edit"></i></button>
                                            @endif
                                        </td>
                                    </tr>
                                @empty
                        ...
                        <div class="history-cards" aria-hidden="true" style="margin-top:1rem;">
                            @forelse ($leaves as $leave)
                                <div class="history-card">
                                    <div class="hc-top">
                                        <h4>{{ ucfirst(str_replace('_', ' ', $leave->leave_type)) }} <small
                                                style="color:var(--text-secondary)">#{{ $leave->leave_id }}</small></h4>
                                        <span
                                            class="status-pill {{ $leave->status }}">{{ ucfirst($leave->status) }}</span>
                                    </div>
                                    <div class="hc-body">
                                        <div>
                                            <strong>Dates</strong><br>{{ \Carbon\Carbon::parse($leave->start_date)->format('M d') }}
                                            – {{ \Carbon\Carbon::parse($leave->end_date)->format('M d, Y') }}
                                        </div>
                                        <div><strong>Duration</strong><br>{{ $leave->duration }}
                                            day{{ $leave->duration != 1 ? 's' : '' }}</div>
                                    </div>
                                    <div style="margin-top:0.5rem;display:flex;justify-content:flex-end;gap:0.5rem;">
                                        <button class="action-btn view"
                                            onclick="viewLeaveRequest('{{ $leave->leave_id }}')" title="View Details"><i
                ...
                <div class="modal-body">
                    <div class="policy-content">
                        <div class="policy-section">
                            <h4>Overview</h4>
                            <p>This document summarises the company's leave policy. It explains types of leave available,
                                eligibility, entitlements, and the process for applying and approving leave requests.</p>
                        </div>
                        ...
                        <div class="policy-section">
                            ...
                            <ol class="policy-list">
                                ...
                                <li>For urgent leave, contact your manager directly and follow up with a portal request.
                                </li>
                            </ol>
                        </div>
