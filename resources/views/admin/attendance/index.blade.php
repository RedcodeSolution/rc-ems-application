...
<div class="card">
    ...
    <div class="card-body" style="padding:1rem;">
        <div style="display:flex;justify-content:space-between;align-items:center;gap:1rem;margin-bottom:1rem;">
            <div>
                <p style="margin:0;color:var(--text-secondary);">Admin view — name, check in, check out, status, total hours, date</p>
            </div>
            <div style="display:flex;gap:0.5rem;align-items:center;">
                <input id="adminSearch" class="form-control" placeholder="Search name or date..." style="min-width:220px;">
...
<script>
(function(){
    const API = '/admin/attendances'; // adjust if needed
    ...
    window.openAdminDetails = function(id){
        const row = Array.from(document.querySelectorAll('#attendanceTbody tr')).find(r => r.style.display !== 'none' && (id === '' || r.innerHTML.includes(id)));
        if(row){
            ...
            document.getElementById('adminModalBody').innerHTML = `<p style="color:var(--text-secondary)">No details available locally. Implement backend detail endpoint to load full details.</p>`;
        }
        document.getElementById('adminAttendanceModal').classList.add('active');
    };

    window.closeAdminModal = function(){ document.getElementById('adminAttendanceModal').classList.remove('active'); };
